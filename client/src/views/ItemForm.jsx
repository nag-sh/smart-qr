...
import {
  Camera, RefreshCw, Sparkles, Tag, Type, FileText, Plus, X, Save, ArrowLeft,
  AlertCircle, Upload
} from 'lucide-react';
...
export default function ItemForm({
  ...
  const startInlineCamera = async (facingMode = 'environment') => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      ...
      if (isEdit) setShowImagePicker(false);
      triggerFilePicker();
      return;
    }
    ...
    try {
      ...
      console.warn('Preferred camera unavailable, trying fallback:', err);
          ...
          <div className={`relative bg-slate-950 overflow-hidden rounded-2xl ${useInlineCamera ? 'aspect-video' : 'aspect-square'}`}>
            {useInlineCamera ? (
              <InlineCamera
                ...
                disabled={isCameraDisabled}
                captureLabel={isCreate ? 'Snap' : 'Snap'}
                uploadLabel="Upload File"
              />
            ) : displayImageSrc ? (
              <>
                <img src={displayImageSrc} alt="Item Preview" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-slate-950/65 opacity-0 hover:opacity-100 flex flex-col items-center justify-center transition-opacity gap-2.5">
                  ...
                  <button
                    type="button"
                    onClick={isEdit ? () => setShowImagePicker(true) : triggerFilePicker}
                  ...
                  >
                    {isEdit ? 'Change Photo' : 'Upload File'}
                  </button>
                </div>
              </>
            ) : compressing ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center space-y-3 text-center text-slate-400 p-4">
                <div className="flex flex-col gap-2 px-4 w-full max-w-[240px]">
                  ...
                  >
                    {isEdit ? 'Select Photo Source' : 'Select File'}
                  </button>
                </div>
                <span className="block text-[9px] text-slate-500 max-w-[240px] mx-auto leading-normal">
                  WebRTC camera interface for mobile and desktop web browsers
                </span>
              </div>
            )}
